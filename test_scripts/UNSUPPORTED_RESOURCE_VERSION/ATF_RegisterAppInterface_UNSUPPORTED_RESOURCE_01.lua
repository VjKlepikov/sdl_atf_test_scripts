...
--------------------------------------------------------------------------------
--Precondition: preparation connecttest_RAIttsUnsupported.lua
commonPreconditions:Connecttest_without_ExitBySDLDisconnect_WithoutOpenConnectionRegisterApp("connecttest_RAIttsUnsupported.lua")

f = assert(io.open('./user_modules/connecttest_RAIttsUnsupported.lua', "r"))
 ...
 -- update hmiCapabilities in UI.GetCapabilities
	...
	if ResultPattern2 == nil then
    	print(" \27[31m ExpectRequest TTS.GetLanguage call is not found in /user_modules/connecttest_RAIttsUnsupported.lua \27[0m ")
  	else
 ...
 -- update hmiCapabilities in TTS.SetGlobalProperties
	...
	if ResultPattern2 == nil then
    	print(" \27[31m ExpectRequest TTS.SetGlobalProperties call is not found in /user_modules/connecttest_RAIttsUnsupported.lua \27[0m ")
  	else
...
-- update hmiCapabilities in TTS.ChangeRegistration
	...
	if ResultPattern2 == nil then
    	print(" \27[31m ExpectRequest TTS.ChangeRegistration call is not found in /user_modules/connecttest_RAIttsUnsupported.lua \27[0m ")
  	else
...
-- update hmiCapabilities in TTS.GetSupportedLanguages
	...
	if ResultPattern2 == nil then
    	print(" \27[31m ExpectRequest TTS.GetSupportedLanguages call is not found in /user_modules/connecttest_RAIttsUnsupported.lua \27[0m ")
  	else
...
-- update hmiCapabilities in TTS.GetCapabilities
	...
	if ResultPattern2 == nil then
    	print(" \27[31m ExpectRequest TTS.GetCapabilities call is not found in /user_modules/connecttest_RAIttsUnsupported.lua \27[0m ")
  	else
...
-- update hmiCapabilities in TTS.IsReady
	...
	if ResultPattern2 == nil then
    	print(" \27[31m ExpectRequest TTS.IsReady call is not found in /user_modules/connecttest_RAIttsUnsupported.lua \27[0m ")
  	else
	    fileContent  =  string.gsub(fileContent, pattern1, 'ExpectRequest("TTS.IsReady", true, { available = false })')
  	end


f = assert(io.open('./user_modules/connecttest_RAIttsUnsupported.lua', "w"))
f:write(fileContent)
f:close()
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------

Test = require('user_modules/connecttest_RAIttsUnsupported')
...
local file_connection  = require('file_connection')
	...
	-- Precondition: removing user_modules/connecttest_RAIttsUnsupported.lua
	function Test:Precondition_remove_user_connecttest()
	 	os.execute( "rm -f ./user_modules/connecttest_RAIttsUnsupported.lua" )
	end

	--Requirement id in JAMA/or Jira ID: SDLAQ-CRS-1328

	--Verification criteria: When ttsName is sent and TTS isn't supported on current HMI, UNSUPPORTED_RESOURCE is returned as a result of request. Info parameter provides additional information about the case. General request result success=true in case of no errors from other components.
