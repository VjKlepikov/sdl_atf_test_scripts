---------------------------------------------------------------------------------------------
-- Requirement summary:
-- [APPLINK-28542]: [Policies] External UCS: PreloadedPT with "disallowed_by_external_consent_entities_off" struct with invalid type of params
--
-- Description:
-- In case:
-- SDL uploads PreloadedPolicyTable with "disallowed_by_external_consent_entities_off:
...
local function sequence(desc, updateFunc)
  ...
  function Test:CheckLog()
    local result = testCasesForPolicySDLErrorsStops.ReadSpecificMessage("Parsed table is not valid", startLine)
    if result ~= true then
