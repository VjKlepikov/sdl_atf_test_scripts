...
-- SDL invalidates notification OnAppPermissionConsent due to invalid value type of parameter EntityStatus
--
-- Used preconditions
...
-- (parameter EntityStatus has invalid value, rest of params present and within bounds)
...
local params_invalid_data = {
  {param = "invalidValue", comment = "NotExistingEnum"},
  {param = 1.32, comment = "Float"},
  {param = 132, comment = "Integer"},
  {param = {}, comment = "Empty table"},
  {param = {entityType = 1, entityID = 1}, comment = "NonEmptyTable"},
  {param = "", comment = "Empty"},
  {param = nil, comment = "Null"}
}
...
for i = 1, #params_invalid_data do
  Test["OnAppPermissionConsent_with_EntityStatus_is_" .. params_invalid_data[i].comment] = function(self)
    self.hmiConnection:SendNotification("SDL.OnAppPermissionConsent", {
      ...
      externalConsentStatus = {
          {entityType = 24, entityID = 94, status = params_invalid_data[i].value}
        },
      source = "GUI"
      })
  ...
  Test["GetListofPermissions_with_EntityStatus_is_" .. params_invalid_data[i].comment] = function(self)
    local request_id = self.hmiConnection:SendRequest("SDL.GetListOfPermissions", {appID = hmi_app_id})
    EXPECT_HMIRESPONSE(request_id,{
      result = {
        code = 0, 
