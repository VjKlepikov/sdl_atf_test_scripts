...
-- SDL invalidates notification OnAppPermissionConsent due to invalid value type of parameter entityType
--
-- Used preconditions
...
-- (parameter entityType has invalid value, rest of params present and within bounds, EntityStatus = 'ON')
-- 2. HMI sends to SDL GetListOfPermissions (appID)
--
-- Expected result:
-- 1. SDL doesn't receive updated Permission items and consent status
-- 2. SDL sends to HMI empty array of <externalConsentStatus>
...
local params_invalid_data = {
  {value = "invalidValue", comment = "String"},
  {value = 1.32, comment = "Float"},
  {value = {}, comment = "EmptyTable"},
  {value = {entityType = 1, entityID = 1}, comment = "NonEmptyTable"},
  {value = -1, comment = "OutOfLowerBound"},
  {value = 129, comment = "OutOfUpperBound"},
  {value = "", comment = "Empty"},
  {value = nil, comment = "Null"}
}
...
for i = 1, #params_invalid_data do
  Test["OnAppPermissionConsent_with_entityType_is_" .. params_invalid_data[i].comment] = function(self)
    self.hmiConnection:SendNotification("SDL.OnAppPermissionConsent", {
      ...
      externalConsentStatus = {
          {entityType = params_invalid_data[i].value, entityID = 94, status = "ON"}
        },
      source = "GUI"
      })
  ...
  Test["GetListofPermissions_with_entityType_is_" .. params_invalid_data[i].comment] = function(self)
    local request_id = self.hmiConnection:SendRequest("SDL.GetListOfPermissions", {appID = hmi_app_id})
    EXPECT_HMIRESPONSE(request_id,{
      result = {
        code = 0, 
