...
-- SDL invalidates notification OnAppPermissionConsent due to missing mandatory parameter entityID
--
-- Used preconditions
...
-- (mandatory parameter entityID is missed, rest of params present and within bounds, EntityStatus = 'ON')
...
function Test:OnAppPermissionConsent_without_entityID()
...
function Test:GetListofPermissions_without_entityID()
  local request_id = self.hmiConnection:SendRequest("SDL.GetListOfPermissions", {appID = hmi_app_id})
  EXPECT_HMIRESPONSE(request_id,{
    result = {
      code = 0, 
