...
-- SDL invalidates notification OnAppPermissionConsent due to missing mandatory parameter EntityStatus
--
-- Used preconditions
...
-- (mandatory parameter EntityStatus is missed, rest of params present and within bounds)
...
function Test:OnAppPermissionConsent_without_EntityStatus()
...
function Test:GetListofPermissions_without_EntityStatus()
  local request_id = self.hmiConnection:SendRequest("SDL.GetListOfPermissions", {appID = hmi_app_id})
  EXPECT_HMIRESPONSE(request_id,{
    result = {
      code = 0, 
