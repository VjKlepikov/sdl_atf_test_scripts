...
-- SDL invalidates notification OnAppPermissionConsent due to invalid value type of parameter entityID
--
-- Used preconditions
...
-- (parameter entityID has invalid value, rest of params present and within bounds, EntityStatus = 'ON')
...
for i = 1, #params_invalid_data do
  Test["OnAppPermissionConsent_with_entityID_is_" .. params_invalid_data[i].comment] = function(self)
    self.hmiConnection:SendNotification("SDL.OnAppPermissionConsent", {
      ...
      externalConsentStatus = {
          {entityType = 24, entityID = params_invalid_data[i].value, status = "ON"}
        },
      source = "GUI"
      })
  ...
  Test["GetListofPermissions_with_entityID_is_" .. params_invalid_data[i].comment] = function(self)
    local request_id = self.hmiConnection:SendRequest("SDL.GetListOfPermissions", {appID = hmi_app_id})
    EXPECT_HMIRESPONSE(request_id,{
      result = {
        code = 0, 
