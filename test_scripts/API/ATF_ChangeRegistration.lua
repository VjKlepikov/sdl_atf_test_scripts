...
--TODO debbug after resolving APPLINK-13101
	--Begin Test case ResultCodeCheck.6
		--Description: Check DISALLOWED result code with success false
			...
			--Description: SDL must return "USER_DISALLOWED, success:false" when RPC has not yet received user's consents.
				local GroupId
				function Test:Precondition_UserDisallowedPolicyUpdate()
					...
					:Do(function(_,data)
						...
						:Do(function(_,data)
							...
							"files/PTU_ForChangeRegistration.json")
							...
							:Do(function(_,data)
								...
								:Do(function(_,data)
									...
									-- TODO: update after resolving APPLINK-16094 EXPECT_HMIRESPONSE(RequestIdGetListOfPermissions,{result = {code = 0, method = "SDL.GetListOfPermissions", allowedFunctions = {{ name = "New"}}}})
				...
				function Test:ChangeRegistration_UserDissallowedSuccessFalse()
					local paramsSend = changeRegistrationAllParams()
					...
					--mobile side: expect ChangeRegistration response
					self.mobileSession:ExpectResponse(CorIdChangeRegistration, { success = false, resultCode = "USER_DISALLOWED" })
				end

				function Test:Postcondition_AllowedChangeRegistration()
...
----ChangeRegistration: Change language of the active test application using ChangeRegistration request------
-------------------------------------------------------------------------------------------------------------
--Requirement id in JAMA or JIRA:
	--APPLINK-18312: -- Change language of the active test application using ChangeRegistration request
	--APPLINK-18313: -- Change language of HMI(UI) and/or VR/TTS
					 -- Change language of application according with new active language on HMI using ChangeRegistration request
	--APPLINK-18314: -- Change hmiDisplayedLanguageDesired of the application to ES-ES by ChangeRegistration request
					 -- Change HMI (UI) language to ES-ES
					 -- App should not been brought to HMIlevel NONE and OnAppUnregistered notification should not been sent to mobile application (because languages of app and HMI (UI), VR+TTS are equal)
	--APPLINK-18315: -- Change languageDesired of the application to ES-ES by ChangeRegistration request
 					 -- Change VR+TTS language to ES-ES
					 -- App should not been brought to HMIlevel NONE and OnAppUnregistered notification should not been sent to mobile application (because languages of app and HMI(UI), VR+TTS are equal)
	--APPLINK-18316: -- Change App name, VR Synonyms, ngn Media Screen App Name and TTS name sucessfully by ChangeRegistration request
-----------------------------------------------------------------------------------------------
...
--Change Language from HMI:
		--language: "EN-US" or any language
		--component: "UI" or "TTS" (VR/TTS)
		--resultCode: "WRONG_LANGUAGE", "SUCCESS"
		--unregisterApp: true/false
function Test:changeHMILanguage(language, component, resultCode, unregisterApp)

	strAppName=config.application1.registerAppInterfaceParams.appName
	local paramsResponse={language=language}

	--hmi side: Change HMI (UI) language to another one
	if component=="UI" then
		self.hmiConnection:SendNotification("UI.OnLanguageChange", paramsResponse)
	else
		--hmi side: Change HMI (TTS/VR) language to another one
		self.hmiConnection:SendNotification("TTS.OnLanguageChange", paramsResponse)
		self.hmiConnection:SendNotification("VR.OnLanguageChange", paramsResponse)
	end

	--Unregistered app
	if unregisterApp then
		...
		:Do(function(_,data)
			...
			config.application1.registerAppInterfaceParams.languageDesired="EN-US"
			...
			{
				success=true, resultCode=resultCode
			})
			:Timeout(12000)

			--mobile side: SyncProxyTester stays active application on UI (Doesn't send OnHMIStatus(hmiLevel=NONE) to mobile app).
	...
	else --Don't unregistered app
		--hmi side: expect no OnAppUnregistered notification
		EXPECT_HMINOTIFICATION("BasicCommunication.OnAppUnregistered", {appID=self.applications[config.application1.registerAppInterfaceParams.appName], unexpectedDisconnect=false}
		)
		:Times(0)
...
---------------------------------------------------------------------------------------------
	...
	--Test case: APPLINK-18312: TC_ChangeRegistration_01
	--Verification criteria: Change language of the active test application using ChangeRegistration request

	local function APPLINK_18312()

		--Print new line to separate new test cases group
		commonFunctions:newTestCasesGroup("Test cases APPLINK-18312: 09[P][MAN]_TC_Change_language_by_ChangeRegistration")

		-- Description: Unregister app
		commonSteps:UnregisterApplication("APPLINK_18312_Precondition_UnregisterApp")

		-------------------------------------------------------------------------------------------------------------

		-- Description: RegisterAppInterface (...., FR-CA, FR-CA)
		function Test:APPLINK_18312_RegisterAppInterface_FR_CA_FR_CA()

			--mobile side: RegisterAppInterface request
			config.application1.registerAppInterfaceParams.languageDesired="FR-CA"
			config.application1.registerAppInterfaceParams.hmiDisplayLanguageDesired="FR-CA"
			CorIdRegister=self.mobileSession:SendRPC("RegisterAppInterface", config.application1.registerAppInterfaceParams)
			strAppName=config.application1.registerAppInterfaceParams.appName
		...
		-- Description: Activation app from 1.1
		function Test:APPLINK_18312_ActivationApp()
		...
		-- Description: ChangeRegistration (language="EN-US", hmiDisplayLanguage="EN-US") from mobile app with and getting SUCCESS response.
		function Test:APPLINK_18312_ChangeRegistration_EN_US_EN_US()

			local requests=changeRegistrationAllParams()
			requests.language="EN-US"
			requests.hmiDisplayLanguage="EN-US"

			--ChangeRegistration (language="EN-US", hmiDisplayLanguage="EN-US")
			self:changeRegistrationSuccess(requests)
	...
	--Test case: APPLINK-18313: TC_ChangeRegistration_02
	--Verification criteria:
							-- Change language of HMI(UI) and/or VR/TTS
							-- Change language of application according with new active language on HMI using ChangeRegistration request

	local function APPLINK_18313()

		--Print new line to separate new test cases group
		commonFunctions:newTestCasesGroup("Test cases APPLINK-18313: 10[P][MAN]_TC_Change_language_of_UI_and/or_VR/TTS")

		-------------------------------------------------------------------------------------------------------------

		--NOTE: The application has ready activated in post-condition APPLINK_18312

		-------------------------------------------------------------------------------------------------------------

		-- Description: --HMI side: Change HMI (UI) language to DE-DE
						--mobile side: recives OnLanguageChange(EN-US,DE-DE), OnAppUnregistered(LANGUAGE_CHANGE) and OnHMIStatus(NONE, NOT_AUDIBLE, MAIN)
		function Test:APPLINK_18313_ChangeHMILanguageUITo_DE_DE()

			--Change HMI (UI) language to DE-DE
			self:changeHMILanguage("DE-DE", "UI", "WRONG_LANGUAGE", true)

			--mobile side: expect OnLanguageChange(EN-US,DE-DE) and OnAppUnregistered(LANGUAGE_CHANGE)
		...
		-- Description: ChangeRegistration (EN-US, DE-DE)
		function Test:APPLINK_18313_ChangeRegistration_EN_US_DE_DE()

			local requests=changeRegistrationAllParams()
			requests.language="EN-US"
			requests.hmiDisplayLanguage="DE-DE"

			--ChangeRegistration (EN-US, DE-DE)
			self:changeRegistrationSuccess(requests)
		...
		function Test:APPLINK_18313_ActivationApp()
		...
		-- Description: -- Start Step2 sheet
						--HMI side: 7. Change TTS\VR language to DE-DE (on UI)
						--mobile side: recives OnLanguageChange(DE-DE,DE-DE), OnAppUnregistered(LANGUAGE_CHANGE) and OnHMIStatus(NONE, NOT_AUDIBLE, MAIN)
		function Test:APPLINK_18313_ChangeHMILangeuageVRTTSTo_DE_DE()

			--Change HMI (UI) language to DE-DE
			self:changeHMILanguage("DE-DE", "TTS", "WRONG_LANGUAGE", true)

			--mobile side: expect OnLanguageChange(DE-DE,DE-DE) and OnAppUnregistered(LANGUAGE_CHANGE)
			EXPECT_NOTIFICATION("OnLanguageChange",{language="DE-DE", hmiDisplayLanguage="DE-DE"})
			EXPECT_NOTIFICATION("OnAppInterfaceUnregistered",{reason="LANGUAGE_CHANGE"})
		...
		-- Description: ChangeRegistration (DE-DE, DE-DE)
		function Test:APPLINK_18313_ChangeRegistration_DE_DE_DE_DE()

			local requests=changeRegistrationAllParams()
			requests.language="DE-DE"
			requests.hmiDisplayLanguage="DE-DE"

			--ChangeRegistration (DE-DE, DE-DE)
			self:changeRegistrationSuccess(requests)
	...
	--Test case: APPLINK-18314: TC_ChangeRegistration_03
	--Verification criteria:
							-- Change hmiDisplayedLanguageDesired of the application to ES-ES by ChangeRegistration request
							-- Change HMI (UI) language to ES-ES
							-- App should not been brought to HMIlevel NONE and OnAppUnregistered notification should not been sent to mobile application (because languages of app and HMI (UI), VR+TTS are equal)

	local function APPLINK_18314()

		--Print new line to separate new test cases group
		commonFunctions:newTestCasesGroup("Test cases APPLINK-18314: 11[P][MAN]_TC_Change_hmiDisplayedLanguageDesired_by_ChangeRegistration")

		-------------------------------------------------------------------------------------------------------------

		-- Description: --HMI side: Change UI language to EN-US (on UI) for precondtion
		function Test:APPLINK_18314_Precondition_HMIChangeLanguageUITo_EN_US()

			--Change HMI (UI) language to EN-US
			self:changeHMILanguage("EN-US", "UI", "WRONG_LANGUAGE", true)

		end

		-------------------------------------------------------------------------------------------------------------

		-- Description: --HMI side: Activation App for precondtion
		function Test:APPLINK_18314_Precondition_ActivationApp()
		...
		-- Description: ChangeRegistration (EN-US, EN-US) for precondition
		function Test:APPLINK_18314_Precondition_ChangeRegistration_EN_US_EN_US()

			local requests=changeRegistrationAllParams()
			requests.language="EN-US"
			requests.hmiDisplayLanguage="EN-US"

			--ChangeRegistration (EN-US, EN-US)
			self:changeRegistrationSuccess(requests)
		...
		-- Description: --HMI side: Change TTS\VR language to EN-US (on UI) for precondtion
		function Test:APPLINK_18314_Precondition_HMIChangeLanguageVRTTS_EN_US()

			--Change HMI (UI) language to EN-US
			self:changeHMILanguage("EN-US", "TTS", "SUCCESS", false)

		end

		-------------------------------------------------------------------------------------------------------------

		-- Description: ChangeRegistration (language="EN-US", hmiDisplayLanguage="EN-US")
		function Test:APPLINK_18314_ChangeRegistration_EN_US_EN_US()
		...
		-- Description: ChangeRegistration (language="EN-US", hmiDisplayLanguage="ES-ES") from mobile app with and getting SUCCESS response.
		function Test:APPLINK_18314_ChangeRegistration_EN_US_ES_ES()

			local requests=changeRegistrationAllParams()
			requests.language="EN-US"
			requests.hmiDisplayLanguage="ES-ES"

			--ChangeRegistration (language="EN-US", hmiDisplayLanguage="ES-ES")
			self:changeRegistrationSuccess(requests)
		...
		-- Description: --HMI side: Change UI language to ES-ES (on UI)
		function Test:APPLINK_18314_ChangeHMILanguageUITo_ES_ES()

			--Change HMI (UI) language to ES-ES
			self:changeHMILanguage("ES-ES", "UI", "SUCCESS", false)
	...
	--Test case: APPLINK-18315: TC_ChangeRegistration_04
	--Verification criteria:
							-- Change languageDesired of the application to ES-ES by ChangeRegistration request
							-- Change VR+TTS language to ES-ES
							-- App should not been brought to HMIlevel NONE and OnAppUnregistered notification should not been sent to mobile application (because languages of app and HMI(UI), VR+TTS are equal)

	local function APPLINK_18315()

		--Print new line to separate new test cases group
		commonFunctions:newTestCasesGroup("Test cases APPLINK-18315: 12[P][MAN]_TC_Change_languageDesired_by_ChangeRegistration")

		-- Description: --HMI side:  Change UI language to EN-US (on UI)
		function Test:APPLINK_18315_Precondition_HMIChangeLanguageUITo_EN_US()

			--Change HMI (UI) language to EN-US
			self:changeHMILanguage("EN-US", "UI", "SUCCESS", true)
		...
		-- Description: --HMI side: Activation App for precondtion
		function Test:APPLINK_18315_Precondition_ActivationApp()
		...
		-- Description: ChangeRegistration (language="ES-ES", hmiDisplayLanguage="EN-US") from mobile app with and getting SUCCESS response.
		function Test:APPLINK_18315_ChangeRegistration_ES_ES_EN_US()

			local requests=changeRegistrationAllParams()
			requests.language="ES-ES"
			requests.hmiDisplayLanguage="EN-US"

			--ChangeRegistration (language="ES-ES", hmiDisplayLanguage="EN-US")
			self:changeRegistrationSuccess(requests)
		...
		-- Description: --HMI side: 5. Change VR+TTS language to ES-ES (on UI)
						--SyncProxyTester stays active application on UI. VR+TTS language has changed to ES-ES
		function Test:APPLINK_18315_ChangeHMILanguageVRTTSTo_ES_ES()

			--Change HMI (UI) language to EN-US
			self:changeHMILanguage("ES-ES", "TTS", "SUCCESS", false)
	...
	--Test case: APPLINK-18316: TC_ChangeRegistration_05
	--Verification criteria: -- Change App name, VR Synonyms, ngn Media Screen App Name and TTS name sucessfully by ChangeRegistration request

	local function APPLINK_18316()

		--Print new line to separate new test cases group
		commonFunctions:newTestCasesGroup("Test cases APPLINK-18316: 13[P][MAN]_TC_Change_app_name/VR_Synonym/nng/screen/TTS_name_by_ChangeRegistration")

		-- Description: --HMI side:  Change VR+TTS language to EN-US (on UI)
		function Test:APPLINK_18316_Precondition_ChangeHMILanguageVRTTSTo_EN_US()

			--Change HMI (UI) language to EN-US
			self:changeHMILanguage("EN-US", "TTS", "SUCCESS", true)
		...
		function Test:APPLINK_18316_Precondition_ActivationApp()
		...
		-- Description: ChangeRegistration (language="ES-ES", hmiDisplayLanguage="EN-US", appName="new app name", ttsName="new TTS Name1, new TTS Name2", ngnMediaScreenAppName="new ngn name") from mobile app with and getting SUCCESS response.
		function Test:APPLINK_18316_ChangeRegistration_ES_ES_EN_US()

			local requests=changeRegistrationAllParams()
			requests.language="EN-US"
			requests.hmiDisplayLanguage="EN-US"
			requests.appName="new app name"
			requests.ttsName={{text ="new TTS Name1", type ="TEXT",},{text ="new TTS Name2", type ="TEXT",},}
			requests.ngnMediaScreenAppName="new ngn name"
			requests.vrSynonyms={"new vr"}

			--ChangeRegistration (language="ES-ES", hmiDisplayLanguage="EN-US", appName="new app name", ttsName="new TTS Name1, new TTS Name2", ngnMediaScreenAppName="new ngn name") from mobile app with and getting SUCCESS response.
			self:changeRegistrationSuccess(requests)
