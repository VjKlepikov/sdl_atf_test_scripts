...
--[[TODO: update according to APPLINK-14765
		...
		----------------------------------------------------------------------------------------------
			...
			--Description:TC's check all resultCodes values in pair with success value
				...
				--Description: check DISALLOWED result code in case when AlertManeuver RPC contains softButton with SystemAction disallowed by policies assigned to this mobile app

					--Requirement id in JAMA: SDLAQ-CRS-682

					--Verification criteria:  SDL must return "DISALLOWED, success:false" fo AlertManeuver RPC to mobile app IN CASE AlertManeuver RPC contains softButton with SystemAction disallowed by policies assigned to this mobile app.

					function Test:Precondition_DisallowedsoftButtons()
						--hmi side: sending SDL.OnAppPermissionConsent
						self.hmiConnection:SendNotification("SDL.OnAppPermissionConsent", { appID =  self.applications["Test Application"], consentedFunctions = {{ allowed = true, id = groupID, name = "AlertManeuverGroup"}}, source = "GUI"})
						-- end)
